...
`> [!TYPE] 본문` 형식으로 강조 박스를 삽입합니다. TYPE과 본문은 반드시 같은 줄에 작성합니다.

```markdown
> [!info] 참고 정보나 배경 설명을 기술합니다.
> [!tip] 유용한 제안이나 권장 사항을 기술합니다.
> [!warning] 주의가 필요한 사항을 기술합니다.
> [!danger] 즉각 조치가 필요한 심각한 사항을 기술합니다.
```

| 타입 | 색상 | 아이콘 | 용도 |
|------|------|--------|------|
| `[!info]` | 파랑 | ℹ | 참고 정보, 배경 설명 |
| `[!tip]` | 초록 | ✔ | 제안, 권장 사항 |
| `[!warning]` | 주황 | ⚠ | 주의 사항 |
| `[!danger]` | 빨강 | ✕ | 즉각 조치 필요 |
...
본문에 출처나 보충 설명을 달 수 있습니다. 모든 각주는 문서 마지막 '참고문헌' 페이지에 자동 수집됩니다.
...
` ```mermaid ` 코드블록으로 플로우차트·시퀀스 다이어그램·마인드맵을 삽입합니다.
...
| 타입 | 용도 | 레이아웃 |
|------|------|----------|
| `flowchart LR` | 가로 프로세스 흐름 | 컴팩트 가로 |
| `flowchart TD` | 세로 의사결정 흐름 | 용지 가로 50%, 높이 자유 |
| `sequenceDiagram` | 시스템·참여자 간 통신 | 전체 너비, 높이 자유 |
| `mindmap` | 개념·구조 방사형 정리 | 전체 너비, 높이 자유 |

다이어그램 색상은 `PAPYRUS_COLOR_PRIMARY` 기반으로 자동 테마 적용됩니다.
...
| `get_template_guide_tool` | 선택한 템플릿의 섹션 구조·변수·작성 가이드 + 파서 구문 레퍼런스 반환 |
| `get_section_pool_tool` | 커스텀 문서 구성용 섹션 풀 반환 |
| `generate_report_tool` | 마크다운 → A4 HTML 변환 + 구문 검증 + 미리보기 서버 기동 |
| `update_report_tool` | 기존 보고서를 새 마크다운으로 업데이트 |
| `get_report_source` | 저장된 보고서의 원본 마크다운 반환 |
| `list_reports` | 저장된 보고서 목록 반환 |
| `open_dashboard` | 대시보드 브라우저 오픈 |
