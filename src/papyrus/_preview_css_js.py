"""CSS and JS string constants for the Papyrus preview layer.

Extracted from preview.py to keep that module under the line limit.
"""
...
(function() {
  ...
  window.__papyrusDownloadPdf = function() {
    var fname = window.__papyrusFilename || '';
    if (!fname) { window.print(); return; }
    var btn = document.getElementById('papyrus-pdf-btn');
    if (btn) btn.textContent = 'PDF 생성 중...';
    fetch('/export-pdf?file=' + encodeURIComponent(fname))
      .then(function(r) {
        if (!r.ok) { window.print(); return; }
        return r.blob();
      })
      .then(function(blob) {
        if (!blob) return;
        ...
        a.download = fname.replace(/\\.html$/, '.pdf');
        a.click();
      })
      .catch(function() { window.print(); })
      .finally(function() {
        if (btn) btn.textContent = 'PDF 저장';
      });
  };
